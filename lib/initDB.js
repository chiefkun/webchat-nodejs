...
history.sync({force: config.isCreated});
...
module.exports = {
	...
	saveMsg: function(msg, callback){
		history.build({name: msg.name, avatar: msg.avatar, message: msg.msg}).save().then(function(history){
			callback(history);
		});
	},
	getHistory: function(callback){
		history.Project.findAll({order: [['createdAt','DESC']],limit: 10 });
	}
};
